...
int main(void)
{
    int count=0, count2=0, i=0, j=0, flag1=0, strwin=0, strlose=0, maxstrwin=0, maxstrlose=0, maxstrwin2=0, maxstrlose2=0, maxcountwin=0, maxcountlose=0, mincountwin=0, mincountlose=0, totalcountwin=0, totalcountlose=0, wincount=0, losecount=0, losemaxmoneycount=0,  wincount2=0, showGameResultFlag;
    ...
    printf("賭けの途中経過を表示しますか？（表示するなら1、しないなら-1を入力）");
    ...
    while(i < count){//１つ目のループ　決めた試行回数以内で
        ...
        if(showGameResultFlag == 1) {
            printf("ダランベール法1回ごとの、最高bet額:%.1f, 最高持金:%.1f, 最低持金:%.1f, 最高連勝数:%d, 最高連敗数:%d, かかったゲーム数:%d, 勝率:%.2f％, ペイアウト率:%.2f％, 収支:%.1f\n\n", maxbet, maxmoney, minmoney, maxstrwin, maxstrlose, j, winrate, (totalpayout/totalbet)*100, money-budget);//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////  
        }
        /*初期化の必要な変数を初期化*/
        maxbet = 0.0;
        maxmoney = budget;
